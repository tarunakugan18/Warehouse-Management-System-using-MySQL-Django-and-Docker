const manage_suppliers_data_modal = document.getElementById("manage-suppliers-modal");
const delete_confirmation_modal = document.getElementById("delete-confirmation-modal");
const form_suppliers_data_add_btn_container = document.getElementById("form-suppliers-data-add-btn-container");
const form_suppliers_data_update_delete_btns_container = document.getElementById("form-suppliers-data-update-delete-btns-container");
const message_container = document.getElementById("message-container");
const manage_suppliers_data_modal_error_container = document.getElementById("manage-suppliers-data-modal-error-container");
const manage_suppliers_data_modal_content = document.getElementById("manage-suppliers-data-modal-content");
...
const suppliers_table_tbody = document.getElementById("suppliers-table-tbody");
const suppliers_table = document.getElementById("suppliers-table");
...
function previousSection() {
    ...
    edit_btns.forEach(edit_btn => {
        edit_btn.addEventListener('click', function handleClick(event) {
            manageSuppliersModalAllow(this.parentElement.parentElement.getAttribute("data-id"), this.className);
        });
    });
...
function manageSuppliersModalAllow(id, className) {

    if (className === "edit-btn") {

        // Allow update and delete buttons
        form_suppliers_data_update_delete_btns_container.style.display = "flex";
        form_suppliers_data_update_delete_btns_container.style.justifyContent = "flex-end";
        form_suppliers_data_add_btn_container.style.display = "none";
        ...
        manage_suppliers_data_modal_error_container.innerHTML = '';


        const form_data = new FormData();

        form_data.append("supplier-id", id);
        form_data.append("edit-btn", "");

        var ajax_request = new XMLHttpRequest();
        // Get the CSRF token from the cookie
        const csrftoken = getCookie('csrftoken');
        ajax_request.open('POST', submitSuppliersDataURL);
        ...
        ajax_request.onreadystatechange = function() {
            if(ajax_request.readyState == 4 && ajax_request.status == 200) {
                ...
                if(results.error && results.error.length > 0) {
                    ...
                    manage_suppliers_data_modal_content.style.overflowY = "scroll";
                } else {

                    manage_suppliers_data_modal.style.display = "block";
        ...
        // Allow add button
        form_suppliers_data_add_btn_container.style.display = "flex";
        form_suppliers_data_add_btn_container.style.justifyContent = "flex-end";
        form_suppliers_data_update_delete_btns_container.style.display = "none";

        manage_suppliers_data_modal_error_container.innerHTML = '';

        // Supplier ID has to be disabled. Default value set in the database.
        form_supplier_id.value = "";
        form_supplier_id.disabled = true;
        form_supplier_id.style.border = "2px solid grey";
        form_name.value = "";
        form_name.disabled = false;
                    ...
                    ajax_request.onreadystatechange = function () {
                        if (ajax_request.readyState == 4 && ajax_request.status == 200) {
                            ...
                            try {
                                ...
                                if (results.error && results.error.length > 0) {
                                    // Display based on events
                                    manage_suppliers_data_modal_content.scrollTop = 0;
                                    manage_suppliers_data_modal_error_container.innerHTML = '<div class="error-message"><b>Error: </b>' + results.error + '</div>';
                                }
                                if ((results.error && results.error.length === 0) && (results.success && results.success.length === 0)) {
                                    // Display based on events
                                    manage_suppliers_data_modal_content.scrollTop = 0;
                                    manage_suppliers_data_modal_error_container.innerHTML = '<div class="error-message"><b>Error: </b> Please refresh the page and try again.</div>';
                                }
                                if (results.success && results.success.length > 0) {
                                    // Display based on events
                                    manage_suppliers_data_modal_content.scrollTop = 0;
                                    manage_suppliers_data_modal_error_container.innerHTML = '<div class="success-message"><b>Success: </b>' + results.success + ' Refreshing page in 3 seconds.</div>';
        ...
        if(ajax_request.readyState == 4 && ajax_request.status == 200) {
            ...
            try {
                ...
                if (results.success && results.success.length > 0) {
                    // Display based on events
                    delete_confirmation_modal.style.display = "none";
                    manage_suppliers_data_modal_error_container.innerHTML = '<div class="success-message"><b>Success: </b>' + results.success + ' Refresing page in 3 seconds.</div>';
...
document.onkeydown = (evt) => {
    ...
    if (evt.key == 27) {
        if (manage_suppliers_data_modal.style.display === "block") {
            manage_suppliers_data_modal.style.display = "none";
        }
    }
};

window.onclick = (event) => {
    if (event.target == manage_suppliers_data_modal) {
        manage_suppliers_data_modal.style.display = "none";
    }
...
function manageSuppliersModalClose() {
    if (manage_suppliers_data_modal.style.display === "block") {
        manage_suppliers_data_modal.style.display = "none";
    }
}
...
add_btn.addEventListener('click', function handleClick(event) {
    manageSuppliersModalAllow("-1", this.className);
});
...
function displayTable() {
    ...
    databaseData.filter((rowSection, currentIndex) => {
        ...
        if(currentIndex >= startSection && currentIndex < endSection) {
            if (table_filter_input.value.length > 0) {
                if (table_filter_input.value.length > 0 && (rowSection.supplier_id.toString().includes(table_filter_input.value) || rowSection.name.includes(table_filter_input.value) || rowSection.email.includes(table_filter_input.value))) {
                    return true;
                } else {
                    return false;
                }
    ...
    suppliers_table_tbody.innerHTML = results;
}
...
function loadPage() {
    const form_data = new FormData();
    form_data.append("fetch-suppliers-data", "");

    var ajax_request = new XMLHttpRequest();
    // Get the CSRF token from the cookie
    const csrftoken = getCookie('csrftoken');
    ajax_request.open('POST', fetchSuppliersDataURL);
    ...
    ajax_request.onreadystatechange = function() {
        if(ajax_request.readyState == 4 && ajax_request.status == 200) {
            ...
            if (results.error.length > 0) {
                ...
                suppliers_table.style.display = "none";
            } else {
                ...
                document.querySelectorAll('#suppliers-table thead tr th').forEach(tb => {
